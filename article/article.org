...
It continuously polls the Kafka consumer for ~Ping~ messages, and for each ~Ping~ message is spawns a new ~StateMachineRunner~.
...
#+begin_src rust -r -n :exports code
  ...
  // Construct the initial state with the pong span
  let initial_state: BoxedState<Types> = Box::new(ListeningForPing::new(pong_span));
